...
float pressione, pressioneMax = leggi_pressione();  // Valore della pressione calcolata, valore massimo raggiunto
...
const int T_OFF_MIN = 200; // Tempo off minimo selezionabile
...
void loop() {
    ...
    if (!iniezione) {
        ...
        switch (funzione) {
            ...
            case 6:
                lcd.setCursor(0,0);
                lcd.print("6)AZZERA PR MAX ");
                if (digitalRead(BUTTON4)) {
                    while (digitalRead(BUTTON4)) {
                        lcd.setCursor(0,0);
                        lcd.print("AZZERA          ");
                    }
                    pressioneMax = leggi_pressione();
                    funzione = 0;
                }
                break;
        }
...
float leggi_pressione() {
    ...
    if (lettura > pressioneMax) {
        pressioneMax = lettura;
    }
